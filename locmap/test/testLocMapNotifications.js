        ...
        lmHelpers.create3LocMapUsersWithApnTokens(test, locMapRestApi, testUserEmail, testUserEmail2, testUserEmail3, function(userDatas) {
            lmHelpers.getCacheWithUser(test, locMapRestApi, userDatas[1].id, function(cache2) {
                lmHelpers.getCacheWithUser(test, locMapRestApi, userDatas[2].id, function(cache3) {
                    locMapRestApi.allowToSeeUserLocation(userDatas[1].id, cache2, {emails: [testUserEmail]}, function(status) {
                        test.equal(status, 200);
                        locMapRestApi.allowToSeeUserLocation(userDatas[2].id, cache3, {emails: [testUserEmail]}, function(status2) {
                            test.equal(status2, 200);
                            locMapRestApi.requestUserLocationUpdates(userDatas[0].id, function(status3) {
            ...
            lmHelpers.getCacheWithUser(test, locMapRestApi, userDatas[1].id, function(cache2) {
                lmHelpers.getCacheWithUser(test, locMapRestApi, userDatas[2].id, function(cache3) {
                    locMapRestApi.allowToSeeUserLocation(userDatas[1].id, cache2, {emails: [testUserEmail]}, function(status) {
                        ...
                        locMapRestApi.allowToSeeUserLocation(userDatas[2].id, cache3, {emails: [testUserEmail]}, function(status2) {
                            test.equal(status2, 200);
                            locMapRestApi.changeUserLocation(userDatas[1].id, cache2, lmHelpers.locMapReport1, function(status3) {
                                test.equal(status3, 200);
                                locMapRestApi.requestUserLocationUpdates(userDatas[0].id, function(status4) {
            ...
            lmHelpers.getCacheWithUser(test, locMapRestApi, userDatas[1].id, function(cache2) {
                lmHelpers.getCacheWithUser(test, locMapRestApi, userDatas[2].id, function(cache3) {
                    locMapRestApi.allowToSeeUserLocation(userDatas[1].id, cache2, {emails: [testUserEmail]}, function(status) {
                        ...
                        locMapRestApi.allowToSeeUserLocation(userDatas[2].id, cache3, {emails: [testUserEmail]}, function(status2) {
                            test.equal(status2, 200);
                            locMapRestApi.setUserVisibility(userDatas[2].id, cache3, {'visibility': false}, function(status3) {
                                test.equal(status3, 200);
                                locMapRestApi.requestUserLocationUpdates(userDatas[0].id, function(status4) {
                                    test.equal(status4, 200);
