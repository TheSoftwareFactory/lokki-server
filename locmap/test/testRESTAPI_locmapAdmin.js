...
module.exports = {
    ...
    setAndGetCrashReports: function (test) {
        ...
        lmHelpers.createLocMapUser(test, testUserEmail, 'dev1', function (auth, reply) {
            var report = {'osType': osType, osVersion: 'foobar', lokkiVersion: '3.0.0',
                reportTitle: 'title', reportData: 'data'},
                authWithData = JSON.parse(JSON.stringify(auth));
            authWithData.data = report;
            lmHelpers.api.post(test, '/v1/crashReport/' + reply.id, authWithData, function () {
                var now = new Date(),
                    year = now.getFullYear(),
                    month = now.getMonth() + 1;
                lmHelpers.api.get(test, '/v1/admin/358405297258/crashReport/' + osType + '/' +
                    year + '/' + month, function (res) {
                        test.ok(lmHelpers.compareCrashReports(res.data,
                            [{osType: osType, report: report}]), 'Report did not match reference.');
                        test.done();
                    });
            });
        });
    ...
    resetUserAccount: function (test) {
        ...
        lmHelpers.createLocMapUser(test, email, 'dev1', function (auth, reply) {
            var userId = reply.id;
            lmHelpers.api.post(test, '/v1/admin/358405297258/accountRecovery',
                {data: {email: email}}, function () {
    ...
    resetNotAllowedOnNormalAccount: function (test) {
        ...
        lmHelpers.createLocMapUser(test, email, 'dev1', function () {
            lmHelpers.api.post(test, '/v1/admin/358405297258/accountRecovery',
                {data: {email: email}}, {status: 401}, function () {
                    lmHelpers.api.post(test, '/v1/signup',
                        {data: {email: email, 'device_id': 'differentid'}},
                        {status: 401}, function () {
                            test.done();
                        });
                });
                    ...
                    lmHelpers.api.post(test, '/v1/user/' + reply.id + '/allow', auth, function () {
                        ...
                        lmHelpers.api.post(test, '/v1/user/' + reply2.id + '/allow', auth2,
                            function () {
                                ...
                                lmHelpers.api.post(test, '/v1/user/' + reply.id + '/apnToken',
                                    auth, function () {
                                        // User 2 is using Android.
                                        auth2.data = {gcmToken: 'token'};
                                        lmHelpers.api.post(test, '/v1/user/' + reply2.id +
                                            '/gcmToken', auth2, function () {
                                                // User 1 is not visible.
                                                auth.data = {visibility: false};
                                                lmHelpers.api.put(test, '/v1/user/' +
                                                    reply.id + '/visibility', auth, function () {
                                                        // User 1 has accessed dashboard today.
                                                        lmHelpers.api.get(test, '/v1/user/' +
                                                            reply.id + '/dashboard', auth,
                                                            function () {
                                                                lmHelpers.api.get(test,
                                                                    '/v1/admin/358405297258/' +
                                                                    'userStats', function (res) {
