...
module.exports = {
    ...
    settingApnOrGcmDoesNotWorkWithoutAuthorization: function (test) {
        ...
        lmHelpers.createLocMapUser(test, testUserEmail, 'dev1', function (auth1, reply1) {
            lmHelpers.api.post(test, '/v1/user/' + reply1.id + '/apnToken',
                { headers: {authorizationtoken: 'wrong'}, data: {apnToken: 'token'}},
                lmHelpers.wrongAuthTokenResult, function () {
                    lmHelpers.api.post(test, '/v1/user/' + reply1.id + '/gcmToken',
                        { headers: {authorizationtoken: 'wrong'}, data: {gcmToken: 'token'}},
                        lmHelpers.wrongAuthTokenResult, function () {
                            test.done();
                        });
                });
    ...
    userPostsHisAppleNotificationDeviceId: function (test) {
        ...
        lmHelpers.createLocMapUser(test, testUserEmail, 'dev1', function (auth1, reply1) {
            ...
            lmHelpers.api.post(test, '/v1/user/' + reply1.id + '/apnToken', auth1, function () {
                test.done();
            });
        });
    },
    ...
    userPostsHisWP8NotificationURL: function (test) {
        ...
        lmHelpers.createLocMapUser(test, testUserEmail, 'dev1', function (auth1, reply1) {
            ...
            lmHelpers.api.post(test, '/v1/user/' + reply1.id + '/wp8NotificationURL', auth1,
                function () {
                    test.done();
                });
        });
